...
2. When Quip Native is the active macOS input source, observe pass-through text input and obtain the destination range and caret position through InputMethodKit. Accessibility remains available for bounded window context and existing-text mode.
...
The judged build targets normal text-input clients in TextEdit, Notes, Chrome, and Safari. It does not promise rich browser editors, Google Docs, terminals, PDFs, password fields, raw-key or canvas editors, games, remote desktops, or universal macOS compatibility.
...
- Package a standalone InputMethodKit Latin input source that passes literal input through, tracks a bounded UTF-16 burst, obtains caret geometry from the active text client, and sends bounded captures to the Tauri engine over loopback.
...
- Use loopback-only MLX services for the 4-bit Qwen3.5 Base and Global adapter endpoints, behind the bundled Rust sidecar contract.
